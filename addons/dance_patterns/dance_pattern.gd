...
func setColour(state = null):
	...
	if state_c == true:
		...
		dance_l_mesh.set_surface_material(0, material)
...
func moveMesh():
	dance_l_mesh.global_transform.origin = dance_l_mesh_mesh.global_transform.origin + Vector3(0.2,0.1,0.1)
	dance_l_mesh.global_transform.origin = dance_l_mesh_mesh.global_transform.origin + Vector3(-0.2,0.1,0.1)
